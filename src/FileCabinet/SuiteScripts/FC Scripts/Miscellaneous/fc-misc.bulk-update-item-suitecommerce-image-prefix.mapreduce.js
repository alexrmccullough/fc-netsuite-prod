...
function getInputData(context) {
    ...
    var sql = `
        SELECT 
            ...
            ImageFiles.hasscimages as itemhasscimages
        FROM
            Item
        LEFT OUTER JOIN
            (
...
function map(context) {
    ...
    if (!result.itemhasscimages && result.brandscimageprefix && result.itemusedefaultbrandimages == 'T') {
        log.debug({ title: 'map - test whether item needs new image', details: 'if statement SUCCEEDED' });
        newItemImagePrefix = result.brandscimageprefix;
        // debug original and new item image prefixes
        log.debug( { title: 'map - new/old prefixes', details: {'newItemImagePrefix': newItemImagePrefix, 'oldItemImagePrefix': result.itemimageprefix} } );
    }
    ...
    try {
        ...
        if (result.itemimageprefix != newItemImagePrefix) {
            log.debug({ title: 'map - test whether NEW matches OLD', details: 'if statement SUCCEEDED. About to write to Reduce stage.' });
