        ...
        (
            CASE
                WHEN (
                    ...
                    AND (Item.custitem_fc_sorule_monday_day IS NOT NULL)
                ) THEN (
                    CASE
                        WHEN BUILTIN.DF(Item.custitem_fc_sorule_monday_day) = 'No Delivery' THEN (TO_DATE('19000101', 'YYYYMMDD'))
                        WHEN (
                        ...
                        WHEN BUILTIN.DF(Item.custitem_fc_sorule_monday_day) = 'Monday' THEN TO_DATE(
                            TO_CHAR(NEXT_DAY(Transaction.shipdate -8, 'MON'), 'YYYYMMDD') ||
                            LPAD(NVL(Item.custitem_fc_sorule_wed_numdays, '0'), 2, '0') ||
                            ':00:00',
                            'YYYYMMDDHH24:MI:SS'
                        )
                        WHEN BUILTIN.DF(Item.custitem_fc_sorule_monday_day) = 'Tuesday' THEN TO_DATE(
                            TO_CHAR(NEXT_DAY(Transaction.shipdate -8, 'TUE'), 'YYYYMMDD') ||
                            LPAD(NVL(Item.custitem_fc_sorule_wed_numdays, '0'), 2, '0') ||
                            ':00:00',
                            'YYYYMMDDHH24:MI:SS'
                        )
                        WHEN BUILTIN.DF(Item.custitem_fc_sorule_monday_day) = 'Wednesday' THEN TO_DATE(
                            TO_CHAR(NEXT_DAY(Transaction.shipdate -8, 'WED'), 'YYYYMMDD') ||
                            LPAD(NVL(Item.custitem_fc_sorule_wed_numdays, '0'), 2, '0') ||
                            ':00:00',
                            'YYYYMMDDHH24:MI:SS'
                        )
                        WHEN BUILTIN.DF(Item.custitem_fc_sorule_monday_day) = 'Thursday' THEN TO_DATE(
                            TO_CHAR(NEXT_DAY(Transaction.shipdate -8, 'THU'), 'YYYYMMDD') ||
                            LPAD(NVL(Item.custitem_fc_sorule_wed_numdays, '0'), 2, '0') ||
                            ':00:00',
                            'YYYYMMDDHH24:MI:SS'
                        )
                        WHEN BUILTIN.DF(Item.custitem_fc_sorule_monday_day) = 'Friday' THEN TO_DATE(
                            TO_CHAR(NEXT_DAY(Transaction.shipdate -8, 'FRI'), 'YYYYMMDD') ||
                            LPAD(NVL(Item.custitem_fc_sorule_wed_numdays, '0'), 2, '0') ||
                            ':00:00',
                            'YYYYMMDDHH24:MI:SS'
                        )
                        WHEN BUILTIN.DF(Item.custitem_fc_sorule_monday_day) = 'Saturday' THEN TO_DATE(
                            TO_CHAR(NEXT_DAY(Transaction.shipdate -8, 'SAT'), 'YYYYMMDD') ||
                            LPAD(NVL(Item.custitem_fc_sorule_wed_numdays, '0'), 2, '0') ||
                            ':00:00',
                            'YYYYMMDDHH24:MI:SS'
                        )
                        WHEN BUILTIN.DF(Item.custitem_fc_sorule_monday_day) = 'Sunday' THEN TO_DATE(
